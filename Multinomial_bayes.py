...
def bayes_params(X, Y):
    ...
    resultsets = {}
    Pc_count = {}
    # calculate each class includes word count except 'null'
    for cla in classes:
      Pc_count[cla] = 0
      for i in range(X.shape[0]):
        for j in range(X.shape[1]):
          if X[i][j] != 'null' and Y[i] == cla:
            Pc_count[cla] += 1
      wordsize += Pc_count[cla]

    # wordtype is used to store the unrepeated word in whole set
    wordtype = []
    for cla in classes:
       rset = {}
         ...
         if Y[i] == cla:
           ...
           for j in range(len(Xi)):
             ...
             if Xi[j] not in rset:
               ...
               rset[Xi[j]] += 1
             if Xi[j] not in wordtype:
               wordtype.append(Xi[j])
               
       resultsets[cla] = rset
       Pc.append(Pc_count[cla] / wordsize)
    Pc = np.array(Pc)
    wordtype_num = len(wordtype) 

    for cla in classes:
      rset = resultsets[cla]
      for i in rset:
        rset[i] = rset[i] / (int(Pc_count[cla]) + wordtype_num)

    return (classes, Pc, resultsets, Pc_count, wordtype_num)
...
def bayes_test(x, classes, Pc, resultsets, Pc_count, wordtype_num):
    """
    Do the Bayes test
    x is one piece of data in the test set
    reutrn: the bayes values for this piece of data
    """
    pdf = multi_var_normal_pdf(x, classes, resultsets, Pc_count, wordtype_num)
    y = []
...
def paired_t_test(X, Y, K, alpha):
    ...
    for i in range(K):
        ...
        classes, Pc, resultsets, Pc_count, wordtype_num = bayes_params(X_i, Y_i)
        ...
        for x in X_test:
          y = bayes_test(x, classes, Pc, resultsets, Pc_count, wordtype_num)
...
dataset = load_data('D:\\Courses\\591-Programming Complex Algorithms\\project\\dressing_multinomial.csv')
(X, Y) = prepare_data(dataset)
K = 30
alpha = 0.95
acceptnum = 0
