...
def paired_t_test(X, Y, K, alpha, isoutputresult = True):
    ...
    for i in range(K):
        ...
        #Y_result = bayes_test(X_test, classes, Pc, mean, cov, False)
        Y_result_naive= bayes_test(X_test, classes, Pc, mean, cov_naive, True)
        ...
        if isoutputresult == True:
          print('sample, full, naive, samplenumber:', i, err_rate_full, err_rate_naive, len(X_test))
    ...
    if isoutputresult: print('bound:', bound)
